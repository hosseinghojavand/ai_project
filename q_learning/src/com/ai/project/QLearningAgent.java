...
public class QLearningAgent extends BaseAgent {
    ...
    private SampleNode [] sampleNodes;
    private QNode [][][] q_table;
    private boolean is_qtable_filled = false;

    private final double EPSILPON = 0;
    private static int N_EPISODES = 1;
    ...
    public Action doTurn(TurnData turnData) {
        ...
        Thread fill_q_table_thread = new Thread(new Runnable() {
            @Override
            public void run() {
               /* Diamond diamond = new Diamond(555 , '4' , 0 , 0);
                Site site = new Site(5555 ,4 ,9 );
                generate_actions_for(turnData , diamond , site);
                actions.clear();*/
        ...
        System.out.println("make rand action -- ajibeh");
        return make_rand_action(turnData);
    }
        ...
        while (N_EPISODES > 0){
            ...
            try {
                ...
                thread = new Thread(new Runnable() {
                    @Override
                    public void run() {
                        try { p[0] = Runtime.getRuntime().exec("python3 AI991-Server.pex"); } catch (Exception e){}
                    }
                });
